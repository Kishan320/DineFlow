...
class PosReportController extends Controller
{
    ...
    public function itemSales(Request $request)
    {
        ...
        $items = PosOrderItem::join('pos_orders', 'pos_orders.id', '=', 'pos_order_items.pos_order_id')
    ...
    public function categorySales(Request $request)
    {
        ...
        $categories = PosOrderItem::join('pos_orders', 'pos_orders.id', '=', 'pos_order_items.pos_order_id')
            ->whereBetween('pos_orders.created_at', [$from . ' 00:00:00', $to . ' 23:59:59'])
            ->whereNotIn('pos_orders.status', ['Cancelled'])
            ->select(
                'pos_order_items.category',
    ...
    public function paymentReport(Request $request)
    {
        ...
        $data = PosOrder::whereBetween('created_at', [$from . ' 00:00:00', $to . ' 23:59:59'])
            ->whereNotIn('status', ['Cancelled'])
            ->select(
    ...
    public function billData(PosOrder $posOrder)
    {
    ...
    public function salesList(Request $request)
    {
        ...
        $query = PosOrder::with('items')
            ...
            ->when($request->order_type && $request->order_type !== 'All',
        ...
        // Summary totals (across full filtered set, not just current page)
        $summaryQuery = PosOrder::query()
            ->when($request->search, fn($q) => $q->where(function ($q) use ($request) {
                $q->where('order_no',    'like', "%{$request->search}%")
                  ->orWhere('invoice_no',   'like', "%{$request->search}%")
                  ->orWhere('customer_name','like', "%{$request->search}%");
